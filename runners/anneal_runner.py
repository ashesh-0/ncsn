...
class AnnealRunner():
    ...
    def train(self):
        ...
        if self.config.data.dataset == 'CIFAR10':
            ...
            dataset = SVHN(os.path.join(self.args.run, 'datasets', 'svhn'),
                           ...
                           download=True,
                           transform=tran_transform)
            test_dataset = SVHN(os.path.join(self.args.run, 'datasets', 'svhn_test'),
                                split='test',
                                download=True,
                                transform=test_transform)
        ...
        sigmas = torch.tensor(
            np.exp(
                np.linspace(np.log(self.config.model.sigma_begin), np.log(self.config.model.sigma_end),
        ...
        for epoch in range(self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                if self.config.training.algo == 'dsm':
                    ...
                    loss = anneal_sliced_score_estimation_vr(score,
                                                             X,
                                                             labels,
                                                             sigmas,
    ...
    def anneal_Langevin_dynamics_inpainting(self,
                                            x_mod,
                                            refer_image,
                                            scorenet,
                                            sigmas,
