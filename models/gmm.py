...
class GMMDistAnneal(object):
    ...
    def log_prob(self, samples, sigma=1):
        ...
        for i in range(len(self.mix_probs)):
            logps.append((-((samples - self.means[i])**2).sum(dim=-1) /
                          (2 * sigma**2) - 0.5 * np.log(2 * np.pi * sigma**2)) + self.mix_probs[i].log())
...
class GMMDist(object):
    ...
    def log_prob(self, samples):
        ...
        for i in range(len(self.mix_probs)):
            logps.append((-((samples - self.means[i])**2).sum(dim=-1) /
                          (2 * self.sigma**2) - 0.5 * np.log(2 * np.pi * self.sigma**2)) + self.mix_probs[i].log())
        logp = torch.logsumexp(torch.stack(logps, dim=0), dim=0)
        return logp
...
class Square(object):
    ...
    def log_prob(self, samples):
        ...
        idx = (samples[:, 0] <= range_th) & (samples[:, 0] >= -range_th) & (samples[:, 1] <= range_th) & (samples[:, 1]
                                                                                                          >= -range_th)
...
class GMM(nn.Module):
    ...
    def forward(self, X):
        energy = (X.unsqueeze(1) - self.mean)**2 / (2 *
                                                    (2 * self.log_std).exp()) + np.log(2 * np.pi) / 2. + self.log_std
